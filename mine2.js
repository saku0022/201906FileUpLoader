...
tm.bombed(function() {
    var app = tm.app.CanvasApp("canvas_wrapper");
    app.resize(BD_WD, BD_HT);
    app.fitWindow();
    ...
    // 一個作っとく
    var x = tm.util.Random.randint(0, BD_WD);
    var y = tm.util.Random.randint(0, BD_HT);
    var crash = createCrashSprite(x, y);
    app.currentScene.addChild(crash);

    app.currentScene.update = function(app) {
        var p = app.pointing;
        if (p.getPointingStart() == true) {
            var crash = createCrashSprite(p.x, p.y);
            this.addChild(crash);
        }
    };
