    ...
    public function execute() {
		...
		foreach( $file_list2 as $file_name){
			...
			{// もしファイルだったら、、
				...
				$file_list3[$i]['name'] = mb_convert_encoding($file_name, "UTF-8", "SJIS");//utf-8になっている

				// ファイルの更新日時を取得
				$file_list3[$i]['date'] = date("Y/m/d H:i", filemtime($dir . "/" . $file_name));
				// ファイルのサイズを取得
		...
		function order_by_desc($a,$b)
		{
			if ( strtotime($a[1]) > strtotime($b[1])){
				...
				$this->smarty->assign('file_list3', $file_list3);
